...
def prepare_timeline_df(df):
    ...
    tmp_df_tmp = pd.pivot_table(
        tmp_df_tmp, aggfunc="sum", values="datasets", index="year"
    ).reset_index(drop=False)
    tmp_df_tmp["type"] = "datasets"
    tmp_df_tmp2 = tmp_df.copy()
    tmp_df_tmp2 = pd.pivot_table(
        tmp_df_tmp2, aggfunc="sum", values="file_number", index="year"
    ).reset_index(drop=False)
    ...
    tmp_df_tmp3 = pd.pivot_table(
        tmp_df_tmp3, aggfunc="sum", values="author_count", index="year"
    ).reset_index(drop=False)
...
def plot_timeline_dataset_files(df):
    ...
    ax = sns.barplot(
        ax=ax1, x="year", y="datasets", hue="type", data=df, palette=palette
    )
    ax.legend([])
    ax = sns.barplot(
        ax=ax2, x="year", y="file_number", hue="type", data=df, palette=palette
    )
...
def plot_origin_count(df):
    ...
    plt.title("File count for each year by origin")
...
def prepare_ext_count_df(df):
    ...
    count_article_df = pd.DataFrame(
        data=count_article_df,
        columns=["year", "author", "ext", "cat", "engine", "size", "dataset id"],
    )
...
def get_info_from_mdp(df):
    ...
    mdp_files_df = mdp_files_df.loc[mdp_files_df["file_type"] == "mdp"].reset_index(
        drop=True
    )
...
def get_info_from_gro(df):
    ...
    gro_files_df = gro_files_df.loc[gro_files_df["file_type"] == "gro"].reset_index(
        drop=True
    )
    ...
    for index, file in gro_files_df.iterrows():
        ...
        try:
            ...
            print(
                f"ERROR for dataset ID: {file['dataset_id']}, file name: {file['file_name']}"
            )
...
def plot_sys_size_count(df):
    ...
    plt.title(
        f"File counts for each size (= number of atoms) extracted from {len(df)} gro files"
    )
